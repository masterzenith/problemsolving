...
/**
 * Time: O(N*logK), where N is the total number of intervals, and K is the total number of employees. This is bcoz
 * we are iterating through the intervals only once(which will take O(N)), and every time we process an interval,
 ...
 * Space: The space complexity of the above algorithm will be O(K) as at any time, the heap will not have more than
 * K elements.
 */
