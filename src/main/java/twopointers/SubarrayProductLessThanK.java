...
/**
 * Time: The main for-loop managing the sliding window takes O(N) but creating subarrays can take up to
 * O(N^2) in the worst case. Therefore overall, our algorithm will take O(N^3)
 * Space: Ignoring the space required for the output list, the algo runs in O(N) space which is used for
 * the temp list.
 * <p>
 * It is not all the Combinations of all elements of the array!
 * For an array with distinct elements, finding all its contiguous subarrays is like finding
 * the number of ways to choose two indices, i and j, in the array such that i <= j.
 ...
 * so overall, our algo space complexity will be O(N^3).
 */
