 ...
 * Output: [5, 6, 9]
 */
public class KClosestElementsTwoPointers {
    ...
    public static void main(String[] args) {
        List<Integer> result = KClosestElementsTwoPointers.findClosestElements(new int[]{5, 6, 7, 8, 9}, 3, 7);
        System.out.println("'K' closest numbers to 'X' are: " + result);

        result = KClosestElementsTwoPointers.findClosestElements(new int[]{2, 4, 5, 6, 9}, 3, 6);
        System.out.println("'K' closest numbers to 'X' are: " + result);

        result = KClosestElementsTwoPointers.findClosestElements(new int[]{2, 4, 5, 6, 9}, 3, 10);
        System.out.println("'K' closest numbers to 'X' are: " + result);
    }
}
/**
 * Time: The time complexity of the above algorithm is O(logN + K). We need O(logN) for
 * Binary Search and O(K)for finding the ‘K’ closest numbers using the two pointers.
